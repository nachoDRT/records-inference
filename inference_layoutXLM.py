...
SUBJECTS_SEMANTIC_JSON = os.path.join(cwd_parent, "7_AdHoc_Git", "records-dataset", "res", "subjects", "subjects_spanish.json")

OBTAIN_METRICS = True
SEND_TO_DATABASE = False


def highlight_ocr_detections(*, img: Image.Image, iter: int, words: list, words_bboxes: list, conf: list):
    ...
    for i, (word, bbox) in enumerate(zip(words, words_bboxes)):
        ...
        img = cv2.putText(img, word, text_start_point, font, font_scale, tuple(a), 2, cv2.LINE_AA)
...
def get_label_2_color():
    ...
    color_dict = {'question':(255, 102, 0), 'answer':(102, 153, 51), 'header':(0, 102, 255), 'other':(204, 0, 153)}
    answers_list = []
    subjects_list = []
    ...
    expanded_labels = get_academic_years_labels(labels=expanded_labels, academic_year_tags=expanded_tags)
...
def generate_aggregated_metrics(*, errors: int, total: int, path: str):
...
def spawn_year():
    ...
    year = {
        "id_curso": random.randint(20, 1000),
        "descripcion": "1º de Bachillerato"
    }

    return year


def create_subject_in_database(*, subject_name: str, id_subject: int, id_academic_year: dict, id_curriculo: int):
    ...
    out_curriculo = requests.post('http://localhost:8000/Subjects/subjects/curriculo', json.dumps(curriculo))
    print("Out_curriculo", out_curriculo)

    subject = {
        "id_asignatura": id_subject,
        "nombre_asignatura": subject_name
    }

    out_subject = requests.post('http://localhost:8000/Subjects/subjects/add_subject', json.dumps(subject))
...
def post_subject_grade(*, num_grade: int, subject_id: int, student_id: str, curriculo_id: int):
...
for file in sorted(os.listdir(real_docs_path)):
    ...
    if file.endswith(".jpg") or file.endswith(".png"):
        ...
        inputs = feature_extractor(img, images_paths=img_path, ocr_azure=True, return_tensors="pt")
        ...
        if OCR_SPY:
            ...
            highlight_ocr_detections(img=ocr_img, iter=iter, words=words[0], words_bboxes=words_bboxes, conf=conf)
        ...
        if SEND_TO_DATABASE:
            ...
            for i, subject in enumerate(grades_output):
                ...
                id = base_id + i
                create_subject_in_database(subject_name=subject,
                                           id_subject=id, 
                                           id_academic_year=academic_year["id_curso"], 
                                           id_curriculo=id_curriculo)
                
                post_subject_grade(num_grade=grades_output[subject],
                                   subject_id=id,
